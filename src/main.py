...
def write_data(df: DataFrame, df_match: DataFrame) -> None:
    ...
    csv_name = "data-import-teams-{}.csv".format(datetime.now().strftime("%Y-%m-%dT%H:%M:%S"))
    bucket.blob(csv_name).upload_from_string(df.to_csv(header=0, index=0), "text/csv")

    csv_name_match = "data-import-matches-{}.csv".format(datetime.now().strftime("%Y-%m-%dT%H:%M:%S"))
